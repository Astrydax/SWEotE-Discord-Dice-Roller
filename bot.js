...
bot.on("message", async message => {
	...
	if (functions.config.patronDiceRole && functions.config.patreonGuild && functions.config[`${channelEmoji}Patreon`]) {
		...
		if (!channelEmoji.includes('Patreon') && functions.config.patronMegaRole && message.guild) if (await functions.checkPatreonServer(bot, message.guild.ownerID)) channelEmoji += 'Patreon';
	}
